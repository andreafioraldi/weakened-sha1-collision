...
static
uint64_t hash_key(const char* k, int use_hash_2, int key_size) {
    /* Taken from http://www.cse.yorku.ca/~oz/hash.html */
    uint64_t hash = 5381;
    uint64_t next;
    int i;
    for (i = 0; i < key_size; ++i, ++k) {
        hash *= 33;
        next = *k;
        //printf("%x\n", next);
        if (use_hash_2) {
...
size_t node_size_opts(HashTableOpts opts) {
    return aligned_size(opts.key_len + 1) + aligned_size(opts.object_datalen);
}
...
HashTableEntry entry_at(const HashTable* ht, size_t ix) {
    ...
    r.ht_data = (void*)( node_data + ix * node_size(ht) + aligned_size(cheader_of(ht)->opts_.key_len + 1) );
    return r;
}
...
void* dht_lookup(const HashTable* ht, const char* key) {
    uint64_t h = hash_key(key, ht->flags_ & HT_FLAG_HASH_2, cheader_of(ht)->opts_.key_len) % cheader_of(ht)->cursize_;
    uint64_t i;
    for (i = 0; i < cheader_of(ht)->cursize_; ++i) {
        HashTableEntry et = entry_at(ht, h);
        if (!et.ht_key) return NULL;
        if (!memcmp(et.ht_key, key, cheader_of(ht)->opts_.key_len)) return et.ht_data;
        ++h;
...
int dht_insert(HashTable* ht, const char* key, const void* data, char** err) {
    ...
    /*if (strlen(key) >= header_of(ht)->opts_.key_len) {
    ...
    uint64_t h = hash_key(key, ht->flags_ & HT_FLAG_HASH_2, header_of(ht)->opts_.key_len) % cheader_of(ht)->cursize_;
    while (1) {
        HashTableEntry et = entry_at(ht, h);
        if (entry_empty(et)) break;
        if (!memcmp(et.ht_key, key, header_of(ht)->opts_.key_len)) {
            return 0;
        }
        ++h;
